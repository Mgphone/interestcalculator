...
type BorrowItem = { id: number; amount: number | string; rate: number | string; };

export default function Home() {
  const [saveItems, setSaveItems] = useState<SaveItem[]>([{ id: 1, amount: '', rate: '' }]);
  const [borrowItems, setBorrowItems] = useState<BorrowItem[]>([{ id: 1, amount: '', rate: '' }]);
  ...
  const handleSaveChange = (id: number, newAmount: number | string, newRate: number | string) => {
  ...
  const handleBorrowChange = (id: number, newAmount: number | string, newRate: number | string) => {
